...
mod test {
    ...
    #[test]
    fn z3_category() {
        let z2_module = Arc::new(CanonZModule::new(vec![3]));

        let bottom_ = bitvec![1, 0, 0, 0, 0, 0, 0, 0, 0];
        let top_ = bitvec![1, 1, 1, 1, 1, 1, 1, 1, 1];
        let zero_ = bitvec![1, 1, 1, 0, 0, 0, 0, 0, 0];
        let zero_dagger_ = bitvec![1, 0, 0, 1, 0, 0, 1, 0, 0];
        let one_ = bitvec![1, 0, 0, 0, 1, 0, 0, 0, 1];
        let two_ = bitvec![1, 0, 0, 0, 0, 1, 0, 1, 0];
        ...
        let two = Relation {
            source: Arc::clone(&z2_module),
            target: Arc::clone(&z2_module),
            matrix_normal: two_.clone(),
            matrix_transposed: two_.clone(),
        };

        let mut hom_sets_fixed_source = HashMap::new();
        hom_sets_fixed_source.insert(
            z2_module.as_ref().clone(),
            vec![bottom, top, zero, zero_dagger, one, two],
        );
